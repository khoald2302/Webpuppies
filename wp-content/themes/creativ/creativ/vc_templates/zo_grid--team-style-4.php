...
?>
<div class="zo-grid-wrapper zo-team-layout-4 <?php echo esc_attr($atts['template']); ?>" id="<?php echo esc_attr($atts['html_id']); ?>">
            ...
            <ul>
                ...
                <?php foreach ($atts['categories'] as $category): ?>
                    <?php $term = get_term($category, $taxonomy);
                     ...
                     ?>
                    <li><a href="#" zo-data-color="<?php echo esc_attr("color-".$term->slug) ?>" style="background: <?php echo esc_attr($zo_color_inline['color']);?> ! important; color: #fff" class="<?php echo esc_attr('category-' . $term->slug); ?>" data-group="<?php echo esc_attr('category-' . $term->slug); ?>">
