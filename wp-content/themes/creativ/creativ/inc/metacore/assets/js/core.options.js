jQuery(document).ready(function($) {
    ...
    /* imageselect   */
     $('.image-field').each(function(e) {
         var _images = $(this).find('input');
         var follow = $(this).attr('data-follow');
         var value = $(this).attr('data-select-value');
         XOptionsFollowImages(_images, follow,value,1);
     })
	/** switch */
	...
	$(document.body).on('click', '.image-field ul li', function(e) {
		...
		$(this).parents('.image-field').find('input').val(value);
         var _images = $(this).parent().parent().find('input');
         var follow = $(this).parent().parent().attr('data-follow');
         var zo_val = $(this).parent().parent().attr('data-select-value');
         var zo_val_s  = jQuery.parseJSON(decodeURIComponent(zo_val));
         var data = jQuery.parseJSON(decodeURIComponent(follow));
        
         XOptionsFollowImages(_images, follow,zo_val,0);
	});
    ...
    function XOptionsFollowImages(element, data,value,start) {
        if (data != undefined) {
            data = jQuery.parseJSON(decodeURIComponent(data));
			var value_data = jQuery.parseJSON(decodeURIComponent(value));
    		var active = false;
            var valueArray = $.map(value_data, function(value, index) {
                return [value];
            });

            var val_element = element.val();
            if(val_element == ''){
              val_element = '0';  
            }
            ...
            if(data) {
                jQuery.each(data, function(index) {
                   if(start == 1){
                       jQuery('' + data[index] + '').addClass('zo-select-off zo-select-image');
                   }
                   "use strict";
                   jQuery.each(valueArray,function(zo_index){

                       if(valueArray[zo_index] == '' || valueArray[zo_index] == null){
                            valueArray[zo_index] = '0';
                        }

					   if (parseInt(val_element) != parseInt(valueArray[zo_index])) {
						   jQuery('' + data[zo_index] + '').css('display', 'none');
					   } else {
						   //	   active = true;
						   jQuery('.zo-select-image').each(function (e) {
							   jQuery(this).addClass('zo-select-off');
							   jQuery(this).removeClass('zo-select-on');

						   });
						   jQuery('' + data[zo_index] + '').removeClass('zo-select-off');
						   jQuery('' + data[zo_index] + '').css('display', 'block');
					   }
				   });
